...
class NeuralNetwork:
    ...
    def __init__(self,
        ...
        self.init_weights_from_hidden_layer_neurons_to_out_layer_neurons(
            out_layer_weights)
        ...
        for inputs in range(len(self.input_layer.neurons)):
            for hidden in range(len(self.hidden_layer.neurons)):
                if not hidden_layer_weights:
                    ...
                    self.hidden_layer.neurons[hidden].weights.append(
                        random.random())
                else:
                    self.hidden_layer.neurons[hidden].weights.append(
                        hidden_layer_weights[weight_no])
                weight_no += 1
        ...
        for output in range(len(self.out_layer.neurons)):
            for hidden in range(len(self.hidden_layer.neurons)):
                if not out_layer_weights:
                    self.out_layer.neurons[output].weights.append(
                        random.random())
                    print("I made it ************************************")
                else:
                    # make sure im accessing the right spot in the array
                    self.out_layer.neurons[output].weights.append(
                        out_layer_weights[weight_num])
                weight_num += 1
   ...
   #**********   TRAIN   *********************************************************************************
    ...
    def train(self, training_inputs, training_outs):
        ...
        print("feed_forward(training_inputs): ",
              self.feed_forward(training_inputs))
        #print("Im here *********************************")
        #out neurons deltas

        pd_errs_wrt_out_neuron_total_net_input = [
            0] * len(self.out_layer.neurons)
        ...
        for output in range(len(self.out_layer.neurons)):
           ...
           # print("p3.1: ", self.out_layer.neurons[output])
            ...
            print("what im looking for: ",
                  pd_errs_wrt_out_neuron_total_net_input[output])

        #move to hidden neuron deltas
        pd_errs_wrt_hidden_neuron_total_net_input = [
        ...
        for out in range(len(self.out_layer.neurons)):
            ...
            for who in range(len(self.out_layer.neurons[out].weights)):
                ...
                pd_err_wrt_weight = pd_errs_wrt_out_neuron_total_net_input[out] * \
                    self.out_layer.neurons[out].calc_pd_total_net_input_wrt_weight(
                        who)
                ...
                self.out_layer.neurons[out].weights[who] -= self.LEARNING_RATE * \
                    pd_err_wrt_weight
        ...
        for hidden in range(len(self.hidden_layer.neurons)):
            ...
            for wih in range(len(self.hidden_layer.neurons[hidden].weights)):
                ...
                pd_err_wrt_weight = pd_errs_wrt_hidden_neuron_total_net_input[hidden] * \
                    self.hidden_layer.neurons[hidden].calc_pd_total_net_input_wrt_weight(
                        wih)
                #print("pd_error_wrt_weight: ", pd_err_wrt_weight[wih])
                #Delta = alpha * pd E at j/ pd w at i
                self.hidden_layer.neurons[hidden].weights[wih] -= self.LEARNING_RATE * \
                    pd_err_wrt_weight
